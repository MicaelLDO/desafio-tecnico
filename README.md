...
- **Alteração de Senha:** Funcionalidade planejada para permitir que usuários logados alterem suas senhas. No momento, ainda não está funcionando, pois enfrentei dificuldades ao tentar interagir com a API utilizando os métodos POST e PUT.
...
- **Atividades 1 e 2** foram concluídas de forma relativamente rápida. A primeira demandou mais tempo no desenvolvimento do HTML e CSS, totalizando cerca de 40 minutos. Já a segunda exigiu mais esforço, principalmente para elaborar a lógica de verificação de palíndromo, o que levou aproximadamente 2 horas no total.
- Na **Atividade 3**, concentrei a maior parte do tempo disponível para codificação. Foquei em estruturar, padronizar e manter o código limpo. As funcionalidades de cadastro e listagem de usuários foram implementadas sem grandes dificuldades. No entanto, outras partes exigiram mais atenção: cometi alguns esquecimentos e enfrentei erros durante o desenvolvimento. A função de troca de senha foi particularmente desafiadora. Mesmo após alguns dias de reflexão e tentativa de implementação, não consegui finalizá-la. Dediquei cerca de uma semana à atividade, considerando os compromissos paralelos que limitaram meu tempo integral de dedicação.
